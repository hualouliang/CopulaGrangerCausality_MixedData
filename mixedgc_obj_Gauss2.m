...
function [lk der]=mixedgc_obj_Gauss2(Y1,Y2,x,porder)

% Cost function for optimization for reduced model (Y2->Y1)
%
% Input:
%     Y1 - Continuous data
%     Y2 - Binary data
...
mu1=x(1)+y1hist*x(2:porder+1)';
tmpp=x(porder+2)+yhist*x(porder+3:v-1)';
...
indxtmp=find(indx21==1);
       ...
       for i=2:porder+1
           fvec(indxtmp,i)=fvec(indxtmp,1).*y1hist(indxtmp,i-1);
       end
       fvec(indxtmp,porder+2)=a4(indxtmp)./(a5(indxtmp)*a3.*a6(indxtmp)).*mu2(indxtmp).*(1-mu2(indxtmp));
       for i=porder+3:v-1
...
indxtmp=find(indx20==1);
       ...
       fvec(indxtmp,porder+2)=-a4(indxtmp)./((1-a5(indxtmp))*a3.*a6(indxtmp)).*mu2(indxtmp).*(1-mu2(indxtmp));
       for i=porder+3:v-1
           fvec(indxtmp,i)=fvec(indxtmp,porder+2).*yhist(indxtmp,i-porder-2);
       end      
