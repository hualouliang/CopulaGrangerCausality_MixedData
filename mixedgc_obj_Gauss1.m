...
function [lk der]=mixedgc_obj_Gauss1(Y1,Y2,x,porder)

%
% Cost function for optimization for reduced model (Y1->Y2)
%
% Input:
%     Y1 - Continuous data
%     Y2 - Binary data
...
tmpp=x(2*porder+2)+y2hist*x(2*porder+3:v-1)';
...
indxtmp=find(indx21==1);
       ...
       for i=2*porder+3:v-1
           fvec(indxtmp,i)=fvec(indxtmp,2*porder+2).*y2hist(indxtmp,i-2*porder-2);
       end       
